    ...
    try {
      ...
      for (i = 0; i < values.length; i++) {
        if (values[i].email == this.$auth.user.email) {
          this.posts = values[i];
          console.log("forms age is", this.posts.forms.age);
          this.value = this.posts.forms.age;
          console.log("page age is", this.value);
        }
      }
    } catch (err) {}
  },
  mounted() {
    ...
    function updateColor() {
      ...
      if (radius >= 95) {
        ...
        holder.selectAll("circle").style("fill", "gray");
      }
    }
  },
  name: "MaskWearing",
